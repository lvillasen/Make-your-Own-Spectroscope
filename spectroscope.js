        ...
        var capture_or_loaded = 0; // 0 for captured, 1 for loaded
        var grey = [];
        var lamda = [];
        var lamda_cal = [];
var Phase_R_DVD = []
...
const Y_base = 0*0.1*L_hor;

const N = 10000;
const diammeter_CD = 12; //cm
const max_of_disk_height = 0.85 // 85% of diammeter
const disk_aperture = 4;

const slit_align_edge_CD = 1.5 // cm
...
// Solicita acceso a la webcam con las restricciones
navigator.mediaDevices.getUserMedia(constraints)
    .then(function(stream) {
        // Usa el flujo de video de alta resolución aquí, por ejemplo, para mostrarlo en un elemento de video
...
context1.drawImage(img, 0,0, img.width, img.height, 0,0,img.width*ratio, img.height*ratio);
...
var data =[trace1,trace2,trace3,trace4,trace5,trace6]
    var layout1 = {
              ...
              title: "Incident Angle wrt Normal = " + theta_i + " degrees"
          };
...
var layout = {
  ...
  width: 500,
  height: 500,
  title:"Tube Length = "+L_hor+ "cm, Tube Diammeter = "+tube_diammeter+ " cm, Grating Angle = " + grating_angle + " degrees",
  font: {
...
const L_hor = parseFloat(document.getElementById("tube_length").value);
const Y_base = 0*0.1*L_hor;
const Height_max_CD = max_of_disk_height*tube_diammeter;
...
const aperture = Y[0]+disk_aperture*Math.cos(grating_angle*Math.PI/180) ;
...
var CD = {
    x: [Y[0],Y[0]+diammeter_CD*Math.cos(grating_angle*Math.PI/180)],
...
var CD_half_point = {
    x: [Y[0]+diammeter_CD/2*Math.cos(grating_angle*Math.PI/180)],
...
var CD_incision = {
    x: [Y[0]+diammeter_CD*Math.cos(grating_angle*Math.PI/180),Math.max.apply(Math, Y)],
    y: [Height_max_CD-diammeter_CD*Math.sin(grating_angle*Math.PI/180),0],
  mode: 'lines',
  name: 'Tube Incision',
  line: {
    color: 'black',
...
const theta = grating_angle *Math.PI/180;
const Y_min = (1-max_of_disk_height)*tube_diammeter/Math.tan(theta)+diammeter_CD/2*Math.cos(theta);
...
for (var i = 0; i < N; i++) {
 ...
 if (y>Y_min && y_old < Y_min){C_half = C[i]}
y_old = y;
}
console.log (C1.length+" "+Y.length)
console.log ("C_half = "+C_half);
...
const l = Y[0]+diammeter_CD/2*Math.cos(grating_angle*Math.PI/180);
var half_point1 = {
x: [C_half],
y: [l],
  mode: 'marker',
...
var data =[trace1,half_point1,half_point2,extreme_point1,extreme_point2,cover1,cover2]
    var layout = {
              xaxis: {
                  //   range: [0, N],
                  title: "Distance along Circumference (cm)",
                  showgrid: true
              },
              yaxis: {
                  //    range: [-1, 1],
                  title: "Height along Tube (cm)",
                  scaleanchor: 'x',
    scaleratio: 1
              },
              shapes: [
    ...
    },
    {
      ...
      line: {
        ...
        width: 2
              ...
              title:"Tube Length = "+L_hor+ "cm, Tube Diammeter = "+tube_diammeter+ " cm, Circumference = " + C[N-1].toFixed(2) + " cm"
          };
